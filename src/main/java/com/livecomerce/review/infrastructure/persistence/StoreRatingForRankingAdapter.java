...
/**
 * Separate copy of the store-rating aggregate, dedicated to {@code analytics}'s
 * ranking job. {@code analytics}'s declared {@code allowedDependencies} only
 * permits {@code store::in}, not {@code store}'s root package — so it cannot
 * use {@code com.livecomerce.store.LoadStoreRatingPort} directly and needs its
 * own copy of the interface (implemented here, same query underneath).
 */
@Component
@RequiredArgsConstructor
class StoreRatingForRankingAdapter implements LoadStoreRatingPort {
